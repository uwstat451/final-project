...
# ------- PRICES: sales-weighted USD/MWh by state (2023) -------
validate_needed(price_path, "Price file")
price_state <- read_excel(price_path, skip = 2) |>
  filter(!is.na(State), nchar(State) == 2) |>
  # OPTIONAL: drop adjustment lines (uncomment next line if you want to exclude)
  # filter(!grepl("(?i)^adjustment", Entity)) |>
  mutate(
    sales_mwh = parse_number(as.character(`Sales (Megawatthours)`)),
    rev_usd   = parse_number(as.character(`Revenues (Thousands Dollars)`)) * 1000
  ) |>
  group_by(State) |>
  summarise(
    price_MWh = sum(rev_usd,  na.rm = TRUE) / sum(sales_mwh, na.rm = TRUE),
    .groups   = "drop"
  ) |>
  arrange(price_MWh)
...
# ------- EMISSIONS: industrial CO2 million tons by state (2023) -------
...
server <- function(input, output, session){
  ...
  output$price_plot <- renderPlot({
    ...
    if (isTRUE(input$avg_price)) {
      p <- p +
        geom_hline(yintercept = avg_price, linetype = "dashed", color = "firebrick") +
  ...
  output$emiss_plot <- renderPlot({
    ...
    if (isTRUE(input$avg_emiss)) {
      p <- p +
        geom_vline(xintercept = avg_million, linetype = "dashed", color = "firebrick") +
